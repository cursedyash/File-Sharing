...
async def varget_(client: Bot, message: Message):
    ...
    if await is_heroku():
        ...
        if check_var in heroku_config:
            ...
            return await message.reply_text({"nil check_var}")
    else:
        ...
        if not output:
            await message.reply_text(f"not applicable {check_var}")
        else:
            return await message.reply_text(
...
async def vardel_(client: Bot, message: Message):
    ...
    if await is_heroku():
        ...
        heroku_config = HAPP.config()
        if check_var in heroku_config:
            await message.reply_text(f"nil {check_var}")
            del heroku_config[check_var]
        else:
            return await message.reply_text(f"nil {check_var}")
    else:
        path = dotenv.find_dotenv("config.env")
        if not path:
            return await message.reply_text(".env file not found.")
        ...
        if not output[0]:
            return await message.reply_text(f"not applicable {check_var}")
        else:
            await message.reply_text(f"not applicable {check_var}")
...
async def set_var(client: Bot, message: Message):
    ...
    if await is_heroku():
        ...
        if to_set in heroku_config:
            await message.reply_text(f"nil {to_set} nil {value}")
        else:
            await message.reply_text(
                f"not applicable {to_set} nil {value}"
            )
        ...
        if dotenv.get_key(path, to_set):
            await message.reply_text(f"NA {to_set} nil {value}")
        else:
            await message.reply_text(
                f"NA {to_set} nil {value}"
            )
        os.system(f"kill -9 {os.getpid()} && bash start")
